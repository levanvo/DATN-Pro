...
const DashboardStatusBill = () => {
    ...
    useEffect(() => {
        if (getStartDate && getEndDate) {// show data selected time
            let bill0: any = 0;
            let bill1: any = 0;
            let bill2: any = 0;
            let bill3: any = 0;
            ...
            dataGetOrder.map((order: any) => {
                ...
                if (order.status == "0" && timeCreateOrder >= timeStart && timeCreateOrder <= timeEnd || order.status == "0" && timeCreateOrder2 == timeStart2 && timeCreateOrder3 == timeStart3 && timeCreateOrder2 == timeEnd2 && timeCreateOrder3 == timeEnd3) {
                    bill0++
                } else if (order.status == "1" && timeCreateOrder >= timeStart && timeCreateOrder <= timeEnd || order.status == "1" && timeCreateOrder2 == timeStart2 && timeCreateOrder3 == timeStart3 && timeCreateOrder2 == timeEnd2 && timeCreateOrder3 == timeEnd3) {
                    bill1++
                } else if (order.status == "2" && timeCreateOrder >= timeStart && timeCreateOrder <= timeEnd || order.status == "2" && timeCreateOrder2 == timeStart2 && timeCreateOrder3 == timeStart3 && timeCreateOrder2 == timeEnd2 && timeCreateOrder3 == timeEnd3) {
                    bill2++
                } else if (order.status == "3" && timeCreateOrder >= timeStart && timeCreateOrder <= timeEnd || order.status == "3" && timeCreateOrder2 == timeStart2 && timeCreateOrder3 == timeStart3 && timeCreateOrder2 == timeEnd2 && timeCreateOrder3 == timeEnd3) {
                    bill3++
                } else if (order.status == "4" && timeCreateOrder >= timeStart && timeCreateOrder <= timeEnd || order.status == "4" && timeCreateOrder2 == timeStart2 && timeCreateOrder3 == timeStart3 && timeCreateOrder2 == timeEnd2 && timeCreateOrder3 == timeEnd3) {
                    bill4++
                };
            });
            ...
            {loadingOrder ? <Loading /> : <div className="flex justify-between rounded-md ORDER select-none">
                <div className={`selector-Views`}>
                    ...
                    <div className="border h-[285px] ml-5">
                        ...
                        <div className="flex space-x-2 m-3">
                            <input className='w-7' type="color" value={"#8DEEEE"} disabled />
                            <p>Đang giao hàng</p>
                        </div>
                        <div className="flex space-x-2 m-3">
                            <input className='w-7' type="color" value={"#EEEE00"} disabled />
                            <p>Đã nhận hàng</p>
                        </div>
                    </div>
                </div>
